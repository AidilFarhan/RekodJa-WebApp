...
export default async function ActionsPage() {
  ...
  const [{ data: applications, error: applicationsError }, { data: events, error: eventsError }, { data: candidates, error: candidatesError }] = await Promise.all([
    ...
    client.from('gmail_scan_candidates').select('message_id, subject, sender_from, snippet, suggested_company, suggested_role, suggested_status, matched_application_id, review_state, internal_date_ms').eq('user_id', user.id).eq('review_state', 'pending'),
  ]);
