...
export default async function Workspace({ children }: { children: React.ReactNode }) {
  ...
  return <SidebarController><aside className="workspace-sidebar"><WorkspaceNav actionsCount={actionsCount} /><div className="sidebar-bottom"><SettingsLink /><p>My Lord! Uplift my heart for me and make my task easy and remove the impediment from my tongue so people may understand my speech. Surah Taha: 25-28</p></div></aside><div className="workspace-body"><div className="workspace-topbar"><Link className="workspace-brand" href="/dashboard/overview"><span className="brand-mark">j</span>Job Tracker <small>{subscribed ? 'Pro' : 'Free'}</small></Link><div className="topbar-right"><NotificationsBell issues={(issues ?? []) as { id: string; message: string }[]} /><Link href="/dashboard/settings"><span className="avatar">{name.split(/\s+/).slice(0,2).map((part: string) => part[0]).join('').toUpperCase()}</span>{name}</Link></div></div><div className="workspace-content">{children}</div></div></SidebarController>;
}
