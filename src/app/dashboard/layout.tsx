...
import { gmailAccessKind } from '@/lib/billing/server';
...
export default async function Workspace({ children }: { children: React.ReactNode }) {
  ...
  const [{ data: applications }, { data: events }, { data: issues }, { data: connection }] = user ? await Promise.all([
    ...
    // The explicit owner filter complements RLS and means this link can only
    // ever point at the signed-in user's saved tracker.
    client.from('sheet_connections').select('spreadsheet_id').eq('user_id', user.id).order('created_at').limit(1).maybeSingle(),
  ...
  // 'open' means the gate is not enforced anywhere yet, which must keep reading
  // as Free exactly as before rather than inventing a new label.
  const accessKind = user ? await gmailAccessKind(client, user) : 'denied';
  const planLabel = accessKind === 'subscription' ? 'Pro' : accessKind === 'beta' ? 'BETA' : 'Free';
