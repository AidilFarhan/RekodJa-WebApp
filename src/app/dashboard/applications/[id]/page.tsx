...
export default async function ApplicationDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <section><Link href="/dashboard">← Back to applications</Link><div className="detail-heading"><div><p className="eyebrow">Application</p><h1>{typedApplication.company}</h1><p>{typedApplication.role}</p></div><StageChanger applicationId={typedApplication.id} stage={typedApplication.stage} /></div><dl className="detail-grid"><div><dt>Date applied</dt><dd>{formatDate(typedApplication.date_applied)}</dd></div><div><dt>Source</dt><dd>{typedApplication.source || '—'}</dd></div><div><dt>Job URL</dt><dd>{typedApplication.job_url ? <a href={typedApplication.job_url} target="_blank" rel="noreferrer">Open listing</a> : '—'}</dd></div></dl><div className="timeline"><h2>Confirmed history</h2>{timeline.length === 0 ? <p className="empty-state">No confirmed events recorded yet.</p> : <ol>{timeline.map((event) => <li key={event.id}><div className="timeline-dot"/><div><strong>{eventLabel(event)}</strong><p>{event.event_status === 'user_confirmed' ? 'User confirmed' : 'Detected observation'} · {formatDate(event.occurred_at)}</p></div></li>)}</ol>}</div><div className="detail-delete"><DeleteApplicationButton applicationId={typedApplication.id} company={typedApplication.company} /></div></section>;
}
